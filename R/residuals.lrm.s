           ...
           which, ...)
{
  ...
  if(type=="ordinary") return(naresid(naa, Y - plogis(L)))
  ...
  if(type=='score.binary') {
    ...
    for(i in which) {
      ...
      names(r) <- lev[-1]
      for(j in 1:k) 
        r[[j]] <- xi*((Y>=j) - plogis(L-cof[1]+cof[j]))
  ...
  if(type=="score") {
    ...
    z <- function(i,k,L,coef) plogis(coef[pmin(pmax(i,1),k)]+L)
  ...
  if(type=="partial")
    {
      ...
      if(k==1) r <- X * matrix(cof, nrow=dx[1], ncol=dx[2], byrow=TRUE) +
        ...
        for(j in 1:k) {
          y <- Y >= j
          p <- plogis(L - cof.int[1] + cof.int[j])
          R[,,j] <- r + (y-p)/p/(1-p)
        }
      }
      if(dopl) {
