...
prModFit <- function(x, title, w, digits=4, coefs=TRUE,
  ...
  for(z in w) {
    ...
    if(type == 'stats') {
      prStats(obj[[1]], obj[[2]], lang=lang, file=file)
      if(lang != 'latex') ct('\n')
      ...
      if(coefs) {
        ...
        if(lang == 'latex') colnames(U) <- c('$\\hat{\\beta}$', 'S.E.', 'Wald $Z$',
                                             'Pr$(>|Z|)$')
        else
          if(lang == 'html') colnames(U) <- c('&beta;&#770;', 'S.E.', 'Wald <i>Z</i>',
                                              'Pr(&#62;&#124;<i>Z</i>&#124;)')
        if(length(errordf))
          colnames(U)[3:4] <-
            switch(lang,
