  ...
  if (!length(at$levels)) {
    ...
    if (ncen[1] + ncen[3] == 0) { # right censoring only
      ...
      if (length(i)) {
        ...
        uncensored[i] <- TRUE
        rt_cens_beyond <- list(
          newlevel = min.outer.censored / mul,
          range = rng
        )
      }
      ...
      if (nponly) {
        return(list(time = s$time / mul, surv = s$surv))
      }
      ...
      ncen2 <- if (all(uncensored)) {
        ...
        c(
          left = sum(is.infinite(y)), right = sum(is.infinite(y2)),
    ...
    if (!requireNamespace("icenReg", quietly = TRUE)) {
      stop("The icenReg package must be installed to use Ocens with interval censoring")
    }
