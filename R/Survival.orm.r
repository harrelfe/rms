Survival.orm <- function(object, precision = 7, ...) {
  ...
  f <- function(times = numeric(0), lp = 0, X = numeric(0),
    ...
    if (len_t) {
      # Round times to correspond to how values and ranges were rounded
      mul <- 10^precision
      times <- round(times * mul) / mul
    }
    ...
    if (parallel) {
      if (length(values) != k) {
        stop(
          "logic error in Survival.orm with parallel=TRUE  k=", k,
    ...
    if (length(values) != k) {
      stop(
        "logic error in Survival.orm  k=", k,
        " length(values)=", length(values)
      )
    }
    ...
    if (!conf.int) {
      ...
      r <- if (forcedf) {
        ...
        structure(w$surv, names = paste0("P(T>", w$time, ")"))
      } else {
        w
      }
      return(r)
    ...
    for (ir in 1:len_ints) {
      ...
      if (np == 0) {
        v <- if (len_ints < 3000) inv[ir, ir] else infoMxop(info, i = i)
      } else {
        Xi <- X[ix, , drop = FALSE]
        if (len_ints < 3000) {
          ...
          v <- Xi %*% infoMxop(info, i = c(i, idx), B = t(Xi))
        }
      }
      se[ir] <- sqrt(v)
    }
