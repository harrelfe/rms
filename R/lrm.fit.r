...
lrm.fit <-
  function(
    x, y, offset = 0, initial,
    opt_method = c(
      "NR", "nlminb", "LM", "glm.fit", "nlm", "BFGS", "L-BFGS-B",
      "CG", "Nelder-Mead"
    ),
    ...
    if (ftdb) {
      w <- llist(opt_method, length(x), dim(x), length(y), length(offset),
        if (!missing(initial)) initial, compstats, ftdb, maxit,
        labels = FALSE
      )
      prn(w, file = "/tmp/z")
    }
    ...
    # Only consider uniqueness of y values to within y.precision decimal places to the right
    ...
    cont <- switch(opt_method,
      ...
      nlminb = list(
        trace = trace, iter.max = maxit, eval.max = maxit,
    ...
    mle <- function(p, init) {
      if (opt_method == "NR") {
        ...
        opt <- nlm(obj, init,
          gradtol = gradtol, iterlim = maxit, print.level = trace,
          hessian = FALSE
        )
        ll <- opt$minimum
        cof <- opt$estimate
        ...
        opt <- nlminb(
          start = init, objective = logl, gradient = grad, hessian = Hess,
          control = cont
        )
        ll <- opt$objective
        cof <- opt$par
      ...
      if (!ok) {
        ...
        if (!length(msg)) {
          msg <- paste(
            "Iterations and/or function and gradient evaluations:",
            paste(it, collapse = " ")
          )
        }
        message(
      ...
      ibeta <- if (p == 0) integer(0) else -(1:k)
      list(
        logl = ll, alpha = cof[ialpha], beta = cof[ibeta], u = gr,
        info = info, iter = it
      )
    } # end mle()
    ...
    if (p > 0) {
      ...
      if (k > 1 && initglm) {
        f <- try(glm.fit(cbind(1e0, x), 1L * (y >= ymed),
          family = binomial(),
          weights = weights, offset = offset,
          singular.ok = FALSE
        ))
...
newtonr <- function(init, obj, grad, hessian, n,
    ...
    while (TRUE) {
      ...
      if (!is.finite(objfnew) || (objfnew > objf + 1e-6)) {
        ...
        if (step_size <= minstepsize) {
          msg <- paste(
            "Step size ", step_size, " has reduced below minstepsize=",
            minstepsize,
  ...
  msg <- paste(
    "Reached", maxit, "iterations without convergence\nChange in -2LL:",
...
lrmstats <- function(y, ymed, p, lp, loglik, u, weights, sumwt, sumwty) {
  ...
  if (p == 0) {
    ...
    rankstats <- c(
      C = a$concordance, # (conc + 0.5 * tiedx) / (conc + disc + tiedx)
