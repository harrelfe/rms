...
for(g in c(2, 4, 8, 16, 32, 64, 128, 256)) {
  ...
  yg <- cut2(y, g=g)
  print(system.time(f <- blrm(yg ~ x, method='optimizing', conc=1)))
}

```
This is impressive.  For g=256 compare with the execution time of the Newton-Raphson method making optimum use of sparse matrices.  Also compare coefficients.  The default Dirichlet distribution concentration parameter for the intercepts is selected to make the posterior means agree with maximum likelihood estimates, sacrificing some performance of posterior modes.  We used instead a concentration parameter of 1.0 for the Dirichlet prior distribution for intercepts, which seems to make optimization agree more with maximum likelihood estimates.
