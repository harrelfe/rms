...
As the `rstan` component of `blrm` fit objects can be tens of megabytes in size. you'll see `rms` utility function `fitIf` used in the examples below.  `fitIf(fitname <- ...)` determines whether the fit has already been run by sensing the existence of a file named like `fitname.rds` in the current working directory, and quickly loads this object if so.  If an optional file named like `fitname-rstan.rds` is found, that file is also loaded and stored as the `rstan` component of the fit.  `fitIf` also stores a much smaller version of the object by creating `fitname.rds` after removing the large `rstan` component.  If `fitname.rds` doesn't already exist, the model fit is run and the entire result is returned to the user.  Also, a new `fitname.rds` is created without the large `rstan` object.  You lose the ability to run the `rms::stanDxplot(..., rstan=TRUE)` on restored objects (when `fitname-rstan.rds` did not exist), so the `stanDxplot` function tries to find an existing trace plot image file corresponding to the current R markdown chunk when the fit object no longer has the `rstan` component.  For most purposes this doesn't matter, because running \code{stanDxplot} using the defaults shows the needed non-burning samples which are always stored in fit objects.  Note that using the `knitr` `cache=TRUE` option does not work well as the cache files for this script were about a GB in size, and the system does not accurately recognize when a model fit hasn't changed and doesn't need to be run when `rstan` is involved.

The `blrm` AR(1) approach is experimental; see warnings in examples below.
...
fitIf(bs <- blrm(y ~ x1 + pol(x2, 2) + x3))
bs
```
...
fitIf(bsup <- blrm(hospdead ~ dzgroup + rcs(crea, 5) + rcs(meanbp, 5), 
	    	  		 data=support))
bsup
...
fitIf(bppo <- blrm(y ~ x, ~ x, priorsd=1000, priorsdppo=1000))
...
fitIf(bmixor <- blrm(imps79o ~ sqrt(Week) * TxDrug + cluster(id), data=d))
bmixor
```
...
Roughly speaking, of the two models, the one with random effects has a probability of 0.95 of being the correct one.  See `rstan::loo` and `loo::loo.array` for details.
...
L <- x + re[id]   # actual logit
y <- ifelse(runif(n * m) <= plogis(L), 1, 0)
fitIf(breb <- blrm(y ~ x + cluster(id)))
breb
par(mfrow=c(2, 2))
...
fitIf(bst <- blrm(y ~ Tx + cluster(id)))
bst
```
...
fitIf(bcf <- blrm(y ~ Tx + cluster(id), data=z))
bcf
...
fitIf(bcft <- blrm(y ~ Tx + time + cluster(id), data=z))
bcft
```
...
zt <- z[time <= first]
fitIf(bnc <- blrm(y ~ Tx + cluster(id), data=zt))
bnc
```
...
fitIf(bnct <- blrm(y ~ Tx + time + cluster(id), data=zt))
bnct
```
...
x <- X[id]
y <- Y[id]
fitIf(bdup <- blrm(y ~ x + cluster(id) + aTime(time), refresh=50))
bdup
```
...
fitIf(bnoar1 <- blrm(y ~ x + cluster(id)))
bnoar1
```
...
```{r fitar1}
fitIf(bar1 <- blrm(y ~ x + cluster(id) + aTime(time)))
```
...
d <- subset(d, id <= 1000)
fitIf(srhs <- blrm(srhs ~ aTime(t) + cluster(id) +
...
fitIf(bmi <- stackMI(y ~ x1 + x2 + x3, blrm, mi, data=d, refresh=50))
stanDx(bmi)
...
`blrm` orthonormalizes the data design matrix to greatly improve posterior distribution sampling in the case of collinearities among predictors (especially among spline basis functions).    The very last parameter in the model is unchanged, but all proceeding parameters must have prior distributions specified with respect to the derived variables.  This is philosophically reasonable because if one has two collinear terms in a model, say terms as simple as $x$ and $x^2$, it is entirely unreasonable to specify independent priors for the corresponding pair of regression coefficients $\beta_1$ and $\beta_2$.  When $\beta_{1} \uparrow$, $\beta_{2} \downarrow$.  To understand at least the scaling part of the translation, we can compare the posterior draws for $\beta$ that pertain to the original $X$ scale with the posterior draws for $\theta$, the parameter values on the QR-decomposition transformed scale.  These $\theta$ values are stored in the `rstan` object that is part of the `blrm` fit object.  Here is an example where an ordinary cubic polynomial is fitted, first with `x` ranging from [-1,1] then [-10, 10].  A binary treatment variable is the last variable in the model, so it will be the only one that is not transformed.

```{r qrtest,w=7,h=5.5}
par(mfrow=c(2,4), oma=c(0,0,0,0), mar=c(4, 4, 0.5, 1))
set.seed(1)
n <- 200
xunscaled <- (1 : n) / n    # runif(n, 0, 1)
tx        <- rep(0:1, n/2)
L         <- 4 * (xunscaled - 0.5) + 0.5 * tx
y         <- ifelse(runif(n) <= plogis(L), 1, 0)
for(k in c(1, 10)) {
  x <- xunscaled * k
  f <- blrm(y ~ pol(x, 3) + tx, iter=500)
  s <- as.matrix(f$rstan)
  th <- s[, paste0('theta[', 1:4, ']')]
  be <- s[, paste0('beta[',  1:4, ']')]
  plot(th[, 1], be[, 1], xlab=expression(theta[1]), ylab=expression(beta[1]))
  plot(th[, 2], be[, 2], xlab=expression(theta[2]), ylab=expression(beta[2]))
  plot(th[, 3], be[, 3], xlab=expression(theta[3]), ylab=expression(beta[3]))
  plot(th[, 4], be[, 4], xlab=expression(theta[4]), ylab=expression(beta[4]))
  abline(a=0, b=1, col=gray(0.7))
}
```
