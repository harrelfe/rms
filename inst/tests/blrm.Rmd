...
mean(g$draws[, 'x2^2'] > 0, na.rm=TRUE)    # longhand calculation
...
The square of the posterior median for $\sigma_\gamma$ is `r median(bmixor$sigmags)^2` which compares well with the `mixor` estimate of 3.774.  The $\hat{\beta}$s also compare very well.  Note that the model is stated differently, which makes two of the intercepts have different meanings across packages.
...
if(length(bar1$rstan)) {
  ...
  par(mfrow=c(2,2), oma=c(0,0,0,0), mar=c(2, 2, 0.5, 1))
  for(chain in 1 : 4) {
    rhos <- z[, chain, 'rho']
